...
#
# (c) embotech GmbH, Zurich, Switzerland, 2014.
...
plt.step(range(0,kmax),X[0,0:kmax],where='post',hold=True)
plt.step(range(0,kmax),X[1,0:kmax],where='post',hold=True)
plt.title('states')
plt.xlim(0,kmax)
...
plt.step(range(0,kmax),U[0,0:kmax],where='post',hold=True)
plt.title('input')
plt.xlim(0,kmax)
